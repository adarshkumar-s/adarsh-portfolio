(() => {
  ...
  const updateQualityState = () => {
    ...
    qualityValue.textContent = png ? "Lossless" : `${quality.value}%`;
  };
  const resetPreview = () => {
    clearUrl(originalUrl);
    clearUrl(optimizedUrl);
    originalUrl = optimizedUrl = "";
    optimizedBlob = null;
    image = null;
    originalPreview.hidden = true;
    optimizedPreview.hidden = true;
    ["originalSize", "originalDimensions", "originalType", "optimizedSize", "reduction", "optimizedType"].forEach(id => {
  ...
  const loadFile = selected => {
    ...
    file = null;
    if (!selected) return;
    if (!selected.type.startsWith("image/")) return setStatus("That file is not an image.", true);
  ...
  const run = () => {
    ...
    optimize.disabled = true;
    download.disabled = true;
    setStatus("Optimizing locally…");

    requestAnimationFrame(() => {
      const max = Number(dimension.value);
      const scale = max && Math.max(image.naturalWidth, image.naturalHeight) > max
      ...
      if (!ctx) {
        optimize.disabled = false;
        return setStatus("Canvas encoding is unavailable in this browser.", true);
      }
  ...
  format.addEventListener("change", updateQualityState);
  ...
  download.addEventListener("click", () => {
    ...
    a.href = optimizedUrl;
    a.download = `${file.name.replace(/\.[^.]+$/, "")}-optimized.${formatName(optimizedBlob.type || format.value).toLowerCase()}`;
    a.click();
  });
